...
class Woolentor_Wl_Custom_Archive_Layout_Widget extends Widget_Base {
    ...
    protected function render( $instance = [] ) {
        ...
        $shortcode = new \WooLentor_WC_Shortcode_Products( $settings, $type );
        $content = $shortcode->get_content( $settings['product_layout'] );

        ?>
            <div class="ht-products" >
                <div id="wl-shop-tab-area-<?php echo $this->get_id(); ?>" class="wl-shop-tab-area <?php if( $settings['woolentor_product_view_mode'] == 'grid' ){ echo 'grid_view'; }else{echo 'list_view'; } ?>">
                    <?php
                        if ( strip_tags( trim( $content ) ) ) {
                            echo $content;
                        } else{
                            echo '<div class="products-not-found"><p class="woocommerce-info">' . esc_html__( 'No products were found matching your selection.','woolentor-pro' ) . '</p></div>';
                        }
                    ?>
                </div>
            </div>
